...
.
L8 (0,0(0),0,0)
note: here T = total count of M , A and N shifts for the saturday till yet and bracket besides M indicates that this L1 has assinged M shift on next upcoming monday also.
...
point4:
i also want to see separately the content of both support table and pending-comp-off list the  dashboard only.

point5:
also i want to have a provision to manually edit the matrix in the dashboard itself, so that if any changes are required i can do it from there only.
...
g)now after saturday we will move to sunday of 08feb2026.
...
if that L1 has M shift assigned on next upcoming monday or not, if yes then we will not assign that L1 to M shift on 07feb2026 and will check for next L1 with least total count of shifts assigned till yet and so on.
d) similarly we will check for sunday of 08feb2026, we will check the support-table-for-sunday and see which L1 has least total count of shifts assigned till yet, and assign that L1 to A shift on 08feb2026 and so on.
e) now after assigning shifts on saturday and sunday we will add those L1s to pending-comp-off list as per point3.
f) now we will fill the remaining days of that week (from 09feb2026 to 13feb2026) by normal logic of shift assignment as per previous discussions.
